...
class CelebA(VisionDataset):
    ...
    def __init__(self, root,
        ...
        self.attr = torch.div(self.attr + 1, 2)  # map from {-1, 1} to {0, 1}
...
def get_celeba(config, eval=False):

    if eval:
        train_ds = None
    else:
        train_ds = CelebA(root='./datasets/', split='train',
    ...
    eval_ds = CelebA(root='./datasets/', split='test',
                      transform=transforms.Compose([
                        transforms.CenterCrop(140),
                        transforms.Resize(config.data.image_size),
                        transforms.ToTensor(),
