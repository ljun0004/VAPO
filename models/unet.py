...
class UNetModel(nn.Module):
	...
	def forward(self, x, timesteps, extra):
		...
		energy = 0.5 * (x - result).pow(2).reshape(len(result), -1).sum(dim=-1, keepdim=True)
...
class MyUnet(nn.Module):
	def __init__(self, config):
		...
		self.model = UNetModel(
								...
								with_fourier_features = config.model.with_fourier_features,
								ignore_time = False,
								input_projection = True,
								nonlinearity = config.model.nonl
							)


	def forward(self, input, cond=None, labels=None):
	  
		# print(input.shape, self.data_dim)
		if self.config.training.augment_t:
			if self.config.training.class_guidance:
				input, cond, _, labels = torch.split(input, [self.data_dim, 1, 1, self.classes], dim=-1)
				conditioning = {"label": labels}
			else:
				input, cond, _ = torch.split(input, [self.data_dim, 1, 1], dim=-1)
				conditioning = {}

		input = input.reshape(-1, self.config.data.channels, self.config.data.image_size, self.config.data.image_size)
		output = self.model(input, cond.squeeze(), extra=conditioning)
